...
double a,b,xo,q;
//-----------------------------------------------------------------------------------------------------------------------------------------------------------//
double g(double x) // nhap ham g(x) x^3 - 4x^2 + x + 1 = 0
{
...
void lapdonhaunghiem(double xo,double q,double epsilon) {
    ...
    out << "\nPhuong pháp lap don giai gan dúng nghiem bài toán f(x) = 0";
    out << "\nInput: ";
    out << "\nPhuong trình: x^3 - 4x^2 + x + 1 = 0";
    out << "\nKhoang cách li nghiem: (a, b) = (" << a << ", " << b << ")";
    out << "\nSai so epsilon: " << epsilon << "\n";
    out << "\nChay code theo công thuc sai so hau nghiem \n";
    out << "\nChon diem Fourier xap xi dau bat kì : " << xo;
    out << "\nGiá tri co q = " << setprecision(20) << q;
    double x = g(xo);
    int step = 1;
    double del = (epsilon * q) / (1 - q);
    out << "\n\nLan lap thu " << step << ": nghiem x = " << fixed << setprecision(15) << x << ",  sai so = " << del;
    ...
    while (fabs(x - xo) >= epsilon) {
        ...
        step += 1;
        out << "\nLan lap thu " << step;
        out << ": nghiem x = " << setprecision(20) << x;
        out << ",  sai so = " << setprecision(20) << fabs(x - xo);
        //printf("\nLan lap thu %d: nghiem x = %.15lf,  sai so = %.15lf ", step, x, del);
    }
    out << "\n\nKet luan: ";
    out << "\nSo lan lap: " << step << "\nNghiem gan dúng: " << x;
